...
flags.DEFINE_string("similarity_input", None, "Similarity input path")
flags.DEFINE_string("similarity_output", None, "Similarity out path")
flags.DEFINE_string("net", "32, 64, 64, 64, 64, 64, 3", "Neural net's depths")
flags.DEFINE_integer("batch_size", 32, "The size of batch images [32]")
flags.DEFINE_boolean("debug", False, "True if debug mode")
flags.DEFINE_boolean("display_path_similar", False, "True if the full path of the similar image should be displayed")
FLAGS = flags.FLAGS
...
def generator(z, latent_c):
    # depths = [256, 512, 1024, 1024, 1024, 1024, 3]
    depths = [int(s) for s in FLAGS.net.split(',')]
    sizes = zip(
...
def discriminator(input, reuse, dropout, int_feats=False, c_dim=None):
    ...
    depths = [32 * 2 ** x for x in range(7)] + [16]
...
def gan(dataset, sess):
    ...
    # Training loop
    for step in range(global_step.eval(), 1 if FLAGS.debug else int(1e5)):
...
def similarity_view(data_json):
    if FLAGS.similarity_output is not None and not os.path.exists(FLAGS.similarity_output):
        os.makedirs(FLAGS.similarity_output)
    ...
    for i in range(len(data_json)):
        ...
        for index, ax in enumerate(fig.axes):
            path = data_json[i][index]["path"]
            path = path.decode(encoding='UTF-8')
            img = cv.imread(path)
            ax.set_axis_off()
            ax.imshow(img)
            # ax.set_title(str(round(data_json[i][index]["distance"], 4)))
            if FLAGS.display_path_similar:
                new_name = str(path)
            else:
                img_name = os.path.splitext(os.path.basename(str(path)))[0]
                # print(img_name)
                time = str(img_name).split("=")
                base_name = os.path.basename(os.path.dirname(str(path)))
                # print(base_name)
                params = str(base_name).split("_")
                # print(params)
                # print(time)
                new_name = "g:{},amp:{}\n,A:{},t:{}".format(params[1], params[3], params[5], time[1])
            ax.set_title(str(new_name), pad=20)
        if FLAGS.similarity_output is not None:
            figure = plt.gcf()  # get current figure
            figure.set_size_inches(20, 15)
            plt.savefig(FLAGS.similarity_output + "/" + str(i) + ".png", bbox_inches='tight', dpi=fig.dpi)

    # plt.tight_layout()
    if FLAGS.similarity_output is None:
        plt.show()
