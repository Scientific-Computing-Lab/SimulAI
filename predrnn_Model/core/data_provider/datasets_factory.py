from core.data_provider import kth_action, mnist, rayleai
...
def data_provider(dataset_name, train_data_paths, valid_data_paths, batch_size,
    ...
    if dataset_name == "rayleai":
        input_param = {'paths': valid_data_list,
                       ...
                       'input_data_type': 'float32',
                       'name': dataset_name + ' iterator'}
        input_handle1 = datasets_map[dataset_name].DataProccess_Rayle(input_param)

        if is_training:
            input_param = {'paths': train_data_list,
                           'image_width': img_width,
                           'minibatch_size': batch_size,
                           'seq_length': seq_length,
                           'input_data_type': 'float32',
                           'name': dataset_name + ' iterator'}
            input_handle = datasets_map[dataset_name].DataProccess_Rayle(input_param)
            train_input_handle = input_handle.get_train_input_handle()
            train_input_handle.begin(do_shuffle=False)
            test_input_handle = input_handle1.get_test_input_handle()
            test_input_handle.begin(do_shuffle=False)
            return train_input_handle, test_input_handle
        else:
            test_input_handle = input_handle1.get_test_input_handle()
            test_input_handle.begin(do_shuffle=False)
            return test_input_handle
    print ("@@ Done datasets_factory.py")